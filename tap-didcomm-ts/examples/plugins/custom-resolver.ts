import { DIDResolver, DIDDocument, Result } from '../../dist';
...
export class CustomDIDResolver implements DIDResolver {
  ...
  constructor() {
    ...
    this.didDocuments.set('did:example:recipient', {
      ...
      keyAgreement: [{
        id: 'did:example:recipient#key-2',
        type: 'X25519KeyAgreementKey2020',
        controller: 'did:example:recipient',
        publicKeyMultibase: 'z6LSfQowQvLkxkXGdUUADZmguszeAKLMwjzLZhM7Jk6oBtfL'
      }]
    });
  }
   ...
   */
  async resolve(did: string): Promise<Result<DIDDocument>> {
    try {
      const document = this.didDocuments.get(did);
      if (!document) {
        return {
          success: false,
          error: new Error(`DID not found: ${did}`)
        };
      }
...
async function example() {
  ...
  if (result.success) {
    ...
    console.error('Failed to resolve DID:', result.error.message);
  }
