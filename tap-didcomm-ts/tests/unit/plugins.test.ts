...
import { MockDIDCommPlugin } from '../../src';
import type { DIDCommPlugin, DIDCommResult, DIDDocument } from '../../src';

describe('DIDComm Plugin', () => {
  let plugin: DIDCommPlugin;

  beforeEach(() => {
    plugin = new MockDIDCommPlugin();
  });
  ...
  describe('Signing', () => {
    ...
    it('should verify signatures', async () => {
      ...
      const verified = await plugin.signer.verify(testData, signed.data!, testKeyId);
  ...
  describe('Encryption', () => {
    ...
    it('should decrypt data', async () => {
      const encrypted = await plugin.encryptor.encrypt(testData, testRecipients);
      const decrypted = await plugin.encryptor.decrypt(encrypted.data!, testRecipients[0]);
